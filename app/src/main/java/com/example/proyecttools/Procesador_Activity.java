...
public class Procesador_Activity extends AppCompatActivity {
    ...
    private int Tabla_resp_p[] = {R.id.rb_one_p, R.id.rb_two_p, R.id.rb_three_p,};                           //Array de opciones de respuesta
    private String[] all_questions;                                                                  //array que contiene las preguntas
    private RadioGroup Rg_resp;                                                                      //radiogroup de respuestas
    private Button bt_next;

    private TextView Tv_pregunta, tv_score, tv_username, tv_maxscore;                                //objeto que guarda el score
    private TextView tv_vidas;
    private MediaPlayer mp_bad, mp_great;
    private int Vidas=0, Score_p=0;           //TODO: sin usar
    private String id_user_p;                              //alojamos nombre usuario gracias a la key
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_procesador_);

// referencias a campos en pantalla
        id_user_p = getIntent().getStringExtra("username");
        all_questions = getResources().getStringArray(R.array.Procesador_question);              //obtenemos array de strings de las preguntas

        //objeto que guarda el nombre de usuario
        TextView tv_nombre = findViewById(R.id.tv_user_three);
        Tv_pregunta = findViewById(R.id.tv_quiz_questions_three);                                       //lugar donde se muestra la pregunta
        Rg_resp = findViewById(R.id.rg_answers_three);                                                  //grupo de opciones_r
        bt_next = findViewById(R.id.bt_next_three);                                                     //boton de avance
        //bt_prev = findViewById(R.id.bt_prev);                                                       //boton de regreso
        tv_score = findViewById(R.id.tv_score_three);
        tv_username = findViewById(R.id.tv_username_three);
        tv_maxscore = findViewById(R.id.tv_maxScore_three);
        tv_vidas = findViewById(R.id.tv_vidas_three);
...
//nombre de usuario
        ...
        tv_nombre.setText(id_user_p);                                                                  //indicamos el lugar en dónde ponemos el texto
        tv_username.setText("Usuario: " + id_user_p);
...
//      mp_bad = MediaPlayer.create(this, R.raw.bad);
    ...
    private void ShowQuestion() {
        ...
        for (int i = 0; i < Tabla_resp_p.length; i++) {                                                //este ciclo recorre la tabla de los botones de las posibles respuestas
            RadioButton rb = findViewById(Tabla_resp_p[i]);                                            //se recorren todos los id y va a obtener el botón que toca
...
//        }
    ...
    private void CheckAnswer() {
        ...
        int ans = -1;
        for (int i = 0; i < Tabla_resp_p.length; i++) {                                                //Bucle que recorre los id de respuestas
            if (Tabla_resp_p[i] == id) {                                                               //el radio group se comunica con la tabla de ids de respuestas y si el numero seleccionado coincide nos quedamos con el numero
    ...
    private void CheckResults() {                                                                    //condicional que almacena los resultados
        ...
        String Alerta = String.format("Puntuación = %d", Score_p);
        ...
        WIN.setPositiveButton(R.string.Finish, new DialogInterface.OnClickListener() {
            @Override
            public void onClick(DialogInterface dialog, int which) {
                Procesador_Activity.this.finish();
            }
        });
        ...
        for  (int i = 0; i < all_questions.length; i++) {                                            //Respuestas, puntaje, vidas
            if (answer_is_correct[i]){                                                               //incrementamos valor de las respuestas
                ...
                Score_p = Correctas;                                                                 //actualizar registros en cada acierto
                if (Correctas == 50) {
                    WIN.create().show();
                }
...
//            if (Score_p > best_score){                                                                     //veroifica que el score sea mayor de otros
//                ContentValues Modificacion = new ContentValues();
//                Modificacion.put("nombre", id_user_p);
//                Modificacion.put("score", Score_p);
//
...
//            Insertar.put("nombre", id_user_p);
//            Insertar.put("score", Score_p);
