...
// referencias a campos en pantalla
        ...
        Vidas_View = findViewById(R.id.iv_vidas);
...
//nombre de usuario
        ...
        tv_username.setText("Usuario: " + id_user);
...
//TODO: PROCESOS NECESARIOS DE LA BASE DE DATOS QUE GUARDA EL SCORE Y EL NOMBRE
        ...
        if (consulta.moveToFirst()) {
            String username = consulta.getString(0);
            String score = consulta.getString(1);
            tv_maxscore.setText("MaxPuntaje =  " + score);
            BD.close();
        } else {
            BD.close();
        }

//TODO: Funcionamiento del score
...
//      mp_bad = MediaPlayer.create(this, R.raw.bad);
    ...
    private void ShowQuestion() {
        ...
        for (int i = 0; i < Tabla_resp.length; i++) {                                                //este ciclo recorre la tabla de los botones de las posibles respuestas
            RadioButton rb = findViewById(Tabla_resp[i]);                                            //se recorren todos los id y va a obtener el botón que toca
    ...
    private void CheckAnswer() {
        ...
        for (int i = 0; i < Tabla_resp.length; i++) {                                                //Bucle que recorre los id de respuestas
            ...
            if (ans == correct_answer) {
                mp_great.start();
                Score++;
                tv_score.setText("Puntaje = " + Score + "/100" );
            }
        }
...
//TODO: Método de checar resultados
    private void CheckResults() {
        ...
        for  (int i = 0; i < all_questions.length; i++) {                                            //el for pasa por todos los elementos y me los trae
            if (answer_is_correct[i]) Correctas++;                                                       //respueestas
            else Incorrectas++;
        }
